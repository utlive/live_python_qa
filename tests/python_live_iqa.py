...
with progressbar.ProgressBar(max_value=n_files, widgets=widgets) as bar:
    for dist in n_dists.keys():
        for i in range(n_dists[dist]):
            ...
            # sims[k + i] = vif(img_ref, img_dist, args.wavelet)
            sims[k + i] = vif_spatial(img_ref, img_dist, sigma_nsq=5, stride=5)
...
savemat('data/live_iqa_' + args.wavelet + '_vifs.mat', {'vifs': sims, 'pcc': pcc, 'srocc': srocc, 'rmse': rmse})
