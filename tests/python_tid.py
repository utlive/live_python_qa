...
n_ref = 25
dist_file_list = sorted(os.listdir(os.path.join(args.path, 'distorted_images')))[:-3]
n_dist = len(dist_file_list)
...
with progressbar.ProgressBar(max_value=n_dist, widgets=widgets) as bar:
    for i in range(1, n_ref + 1):
        ...
        for dist_file in dist_files:
            ...
            sims[k] = vif(img_ref, img_dist, args.wavelet)
            k += 1
...
savemat('data/tid13_' + args.wavelet + '_vifs.mat', {'vifs': sims, 'pcc': pcc, 'srocc': srocc, 'rmse': rmse})