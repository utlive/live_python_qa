...
parser.add_argument("--wavelet", help="Wavelet to use", type=str, default="steerable")
        ...
        while(i_dist < n_dist_files and ref_filename in dist_file_list[i_dist]):
            ...
            while(v_ref.isOpened() and v_dist.isOpened()):
                ...
                if ret_ref and ret_dist:
                    ...
                    spat_scores.append(vif_spatial(y_ref, y_dist, sigma_nsq=5, k=9, stride=5))
                    # temp_scores.append(vif_spatial(y_ref - y_ref_prev, y_dist - y_dist_prev, win))
                    # spat_scores.append(vif(y_ref, y_dist, args.wavelet))
                    # temp_scores.append(vif(y_ref - y_ref_prev, y_dist - y_dist_prev, args.wavelet))
                    y_ref_prev = y_ref
                    y_dist_prev = y_dist
                else:
                    break
            ...
            sims[i_dist] = np.mean(spat_scores)
...
savemat('data/nflx_repo_' + args.wavelet + '_vifs.mat', {'vifs': sim_spat_alls})
...
savemat('data/nflx_repo_' + args.wavelet + '_vifs.mat', {'vifs': sim_spat_alls, 'pcc': pcc, 'srocc': srocc, 'rmse': rmse})
